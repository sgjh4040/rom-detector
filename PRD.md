...
#### 인라인 style 정책 (audit #18, 2026-05-10 추가)

PRD §1 의 "공통 UI(버튼/뱃지/입력창)는 인라인 금지" 원칙은 그대로 유지하되,
실무 현실에 맞춰 다음과 같이 명시한다:

| 인라인 style 종류 | 허용 여부 |
|---|---|
| **공통 UI 컴포넌트** (버튼/뱃지/입력창/페이지 헤더 등 도메인 명칭이 명확한 것) | ❌ 금지 — 클래스로 추출 |
| **3회 이상 반복되는 같은 패턴** (예: `.btn-back`, `.text-secondary` 같은 utility) | ❌ 금지 — 클래스로 추출 |
| **단일 컴포넌트 내부의 1회성 micro-layout** (특정 위치 padding, 카드 height, gap 등) | ✅ 허용 — 인라인 유지 가능 |
| **동적 값 의존 인라인** (props/state/계산식이 들어가는 경우) | ✅ 허용 — 인라인이 더 명확 |

판정 기준: "이 인라인 style 이 다른 컴포넌트에서 같은 의미로 다시 등장할 가능성이 있는가?"
- YES → 클래스 추출
- NO (해당 컴포넌트 internal layout) → 인라인 유지
