    ...
    if (!patientId || history.length === 0) {
        return (
            <div className="container p-8 text-center neumo-inset">
                ...
                <button className="btn btn-primary mt-4" onClick={() => navigate('/')}>메인으로</button>
            </div>
        );
    }
        ...
        <div className="bg-full-viewport page-bg-results pb-20 neumo-container" style={{ minHeight: '100vh', padding: '0 20px 80px', overflow: 'visible' }}>
            <div className="container" style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', flexDirection: 'column', overflow: 'visible' }}>
                ...
                {!showCharts ? (
                    ...
                    <div className="space-y-12 mb-16" style={{ display: 'flex', flexDirection: 'column', gap: '40px' }}>
                        <div className="card neumo-card" style={{ padding: '40px', borderRadius: '40px' }}>
                        ...
                        {JOINTS.map(joint => (
                            <JointTrendCard key={joint.id} joint={joint} history={reversedHistory} />
                        ))}
                    </div>
                )}
