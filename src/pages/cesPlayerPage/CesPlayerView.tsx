// CesPlayerView.tsx — CES 가이드 플레이어 본문 (비어있지 않은 루틴 전용).
// 빈 루틴 가드는 부모 CesPlayerPage 에서 처리 → 여기선 항상 exercises 1개 이상 보장.
...
import { findNearestExerciseStep } from "./stepNavigation";

export const CesPlayerView: React.FC<{ routine: CesRoutine }> = ({ routine }) => {
  ...
  const {
    ...
    restart,
    skipBreak,
  } = useCesPlayer(routine, session?.createdAt);
  ...
  const anchorExerciseStep = isBreak
    ? findNearestExerciseStep(routine.exercises, stepIndex + 1)
    : currentStep;
  const highlightIds =
