...
describe('MobileBottomNav — 환자 유무·측정기록 따른 nav 분기', () => {
  ...
  it('환자 없음: 홈 + 설정 2개만 노출', () => {
    renderNav();
    expect(screen.getByRole('link', { name: /홈/ })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /설정/ })).toBeInTheDocument();
    expect(screen.queryByRole('link', { name: /측정기록/ })).not.toBeInTheDocument();
    expect(screen.queryByRole('link', { name: /^CES$/ })).not.toBeInTheDocument();
  });

  it('환자 있고 측정기록 없음: 홈/측정기록/설정 3개 (CES 미노출)', () => {
  ...
  it('환자 + 측정기록 있음: 홈/측정기록/CES/설정 4개 노출', () => {
    localStorage.setItem(
      'rom_patients',
    ...
    expect(screen.getByRole('link', { name: /홈/ })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /측정기록/ })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /^CES$/ })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /설정/ })).toBeInTheDocument();
  });
